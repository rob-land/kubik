#!/usr/bin/env bash
# build-all.sh — build Kubik flatpak bundles for x86_64 and aarch64.
#
# Usage:
...
#
# Outputs:
#   kubik-x86_64.flatpak
#   kubik-aarch64.flatpak
...
# ── Skip the dependency step when there are no Python deps ────────────────
# Kubik's runtime needs come entirely from org.gnome.Platform, so
# requirements.txt is comments only and the manifest has no python3-deps
# module. Keep the rest of the script identical to the cohort's.
...
if $HAVE_DEPS && { $REGEN || [[ ! -f "$DEPS" ]]; }; then
    if ! $REGEN; then
...
for arch in "${ARCHES[@]}"; do
    ...
    bundle="kubik-${arch}.flatpak"
    echo
    echo "==== Building Kubik for ${arch} ===="
    flatpak-builder --arch="$arch" --repo=repo --force-clean \
        "$builddir" build-aux/flatpak/land.rob.kubik.json
    echo "==== Bundling ${bundle} ===="
    flatpak build-bundle --arch="$arch" repo "$bundle" land.rob.kubik
    ls -lh "$bundle"
done

# ── Optional: install the host-arch bundle ────────────────────────────────
if $INSTALL; then
    bundle="kubik-${HOST_ARCH}.flatpak"
    if [[ -f "$bundle" ]]; then
        echo
